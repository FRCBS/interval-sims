...
extract_type <- function(red.distr, type) {
  ...
  typed.distr <- aggregate(typed$quantity, by = list(typed$date), sum)
  colnames(typed.distr) <- c("date", "pcs")
  # Merge into a whole set with NAs
  typed.distr <- merge(
    x = data.frame(date = all.dates),
    y = typed.distr,
    all.x = TRUE
  )
  # Replace with zeroes
...
aggregate_weekly <- function(series) {
  pcslist <- list()
  weeklist <- list()
  datelist <- list() # Create storage lists
  j <- 0
  k <- 0 # j is for resetting week counter, k is for gathering dates
...
chosen_forecast <- function(chosen.model, series.ts, freq = "monthly") {
  if (freq == "monthly") { # Monthly forecast
    ...
    if (chosen.model == 11) {
      fit <- stl(train25, s.window = "periodic", t.window = 7)
    }
    if (chosen.model == 12) {
      fit <- tbats(train25)
    }
    ...
    if (chosen.model == 20) {
      fit <- stl(train10, s.window = "periodic", t.window = 7)
    }
    if (chosen.model == 21) {
      fit <- tbats(train10)
    }
